...
I then added player behavior such as movement and shooting behavior to the placeholder object. To implement shooting behavior, I created a placeholder object for the player bullet and attached the player bullet script. For player movement, I made use of the RigidBody component and utilized the function movePosition to move the object along the x-axis. For the player’s shooting behavior,  I created a function called Shoot() and inside this function I would instantiate the bullet object, access its RigidBody component, and add a force to it to shoot it upwards. Eventually, I updated this function to accommodate object pooling which I did in a later story. The shoot function would then be called within the Update function inside a conditional statement where Shoot() would only be called if the Space bar was pressed.
Below is a snippet of the player movement script:
![player movement code snippet](/code-summary-snippets-and-gifs/story2/playermovement.png)

Below is a snippet of the player shooting script:
![player shooting code snippet](/code-summary-snippets-and-gifs/story2/shootingbehavior.png)

After implementing player behavior, I realized the object could move out of the screen. To address this, I create boundaries by clamping the player’s position to a maximum and minimum value to prevent the player from leaving the screen.
The snippet below is the code for implementing the boundaries:
![player boundaries (clamping) code snippet](/code-summary-snippets-and-gifs/story2/playerboundaries.png)

The gif below illustrates the initial player behavior that was implemented during this story.
![Initial player behavior gif](/code-summary-snippets-and-gifs/story2/prototypebullet.gif)
...
In a later story, I eventually changed “Time.time” to “Time.timeSinceLevelLoad” on both the script for alien shooting behavior and the lone alien movement.  I realized because Time.time starts from when the program begins, it would cause issues whenever I would play the game again without stopping and restarting the program. Therefore, I updated it to Time.timeSinceLevelLoad so that the time would be restarted every time the scene loaded. The FINAL folder in the repository holds the code snippet of the updated scripts.
![alien shooting code snippet](/code-summary-snippets-and-gifs/story3/alienbulletshooting.png)
...
Below is a snippet of the initial script for alien collision. To see detailed or updated code snippets, check out the folders in this repository.
![collision behavior code snippet](/code-summary-snippets-and-gifs/story3/alienhit.png)

Below is a gif illustrating the initial implementation of opposition and collision behavior.
![initial implementation of opposition and collision behavior](/code-summary-snippets-and-gifs/story3/alienshootingandcollision.gif)
...
For the score and extra lives, I accessed each TextMeshProUGUI component holding the value for the score and number of extra lives then manipulated the text inside to reflect the right values. To do this I first had to convert the text string to an integer, add (for score) or subtract (for number of lives), and then convert the new value back to a string.
Below is the initial code for dynamically changing the score text.
![score text code snippet](/code-summary-snippets-and-gifs/story4/dynamicscoring.png)

Below is a gif of the initial implementation of UI element functionalities.
![UI gif](/code-summary-snippets-and-gifs/story4/INITIALdynamiclivesandscore_playerspawning.gif)

####<a name="scoreGameoverscene"></a> Showing Score in Game Over Scene
I made use of PlayerPrefs to pass on the score value from the game scene to the game over scene.
![PlayerPrefs code snippet](/code-summary-snippets-and-gifs/story4/playerprefs.png)

### <a name="playerextralives"></a>Player Extra Lives
To implement the extra lives, I used a variable to count how many lives were used. I then used a conditional statement checking the number of extra lives used to determine whether to activate the gameObject again. If the condition was true, the gameObject would be set to active again and the number of lives text would be changed to reflect the number of extra lives left. The number indicating how many lives were used would be incremented while the number of lives left (which displays on the game) would be decremented to reflect the change. The placeholder extra life objects would also be affected to reflect the number of extra lives left.
Below is a code snippet of the initial code for implementing the extra lives functionality.
![extra lives code snippet](/code-summary-snippets-and-gifs/story4/initialplayerspawning_and_dynamiclivestext.png)

### <a name="objectpool"></a>Object Pooling and SetActive to False
After some feedback from my instructor, I decided to pool the bullet objects and pre-instantiate a set number of bullet prefabs instead of constantly instantiating bullets every time the function was called. I also decided to change all of Destroy() to SetActive(false) to lessen the load on the program.
Below is a code snippet of the player bullet pool. Here a set number of bullets is added to a list and the function GetPooledPlayerBullets() returns a bullet from the pool.
![player bullet pool code snippet](/code-summary-snippets-and-gifs/story4/playerbulletpool.png)

Below is a code snippet of the updated player Shoot() function that makes use of object pooling. In this function, GetPooledPlayerBullets() is called and the bullet returned is set to active.
![updated player shoot() function code snippet](/code-summary-snippets-and-gifs/story4/updatedplayershootingfunction_forobjectpooling.png)

### <a name="animations"></a>Animations
#### <a name="startsceneanim"></a>Animation on Start Scene (text)
I animated the text of the score table on the start screen. I replicated a typewriter animation using a Coroutine. Below is the initial animation of the score table.
![start screen text animation gif](/code-summary-snippets-and-gifs/story4/startscenetypewritertext.gif)
...
I used a switch statement to change the sprite of the barriers on each collision.  The code snippet of the initial switch statement is below.
![barrier collision code snippet](/code-summary-snippets-and-gifs/story4/barrierCollision.png)

### <a name="timedinactivity"></a>Timed Inactivity
I included a C# script that set game objects to inactive after a period of time. This was used for the lone alien and player bullets that did not collide with anything so that they do not stay active when they leave the screen. 
Below is the code for the timed inactivity.
![timed inactivity code snippet](/code-summary-snippets-and-gifs/story4/timedinactivity.png)

### <a name="finalgame"></a>Final Game
Below is the gif displaying the final game after finishing this story.
![final game gif](/code-summary-snippets-and-gifs/story4/FINALgameplay_score_shooting.gif)
...
On the start and game over scenes, I added a color tint transition to change the color of button text when hovered over. 
Below is a gif illustrating the final animation of the start screen and the color change on hover.
![Scene Transition](/code-summary-snippets-and-gifs/story5/FINALstartScenetoGameTransition.gif)
...
For all code snippets and gifs see the folders in this repository. The “FINAL code snippets and gifs” folder holds screenshots and gifs for the finalized game and the “code snippets and gifs” folder holds snippets and gifs taken during the process of building the game.
[Finalized game code snippets and gifs](/FINAL-code-snippets-and-gifs)
[Code snippets and gifs taken during development](/code-snippets-and-gifs)
